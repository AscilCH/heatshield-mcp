...
async def chat_endpoint(req: ChatRequest):
    ...
    system_prompt = (
        ...
        "   - Spatial routes, walking paths, or zones? Draw the corridors/polygons on the map (`get_walking_route`, `draw_map_layer`, `submit_geospatial_tasks`) and smoothly fly the camera (`set_camera_view`) to the focal area. CRITICAL: You cannot run `get_walking_route` concurrently with `find_cooling_spots` in a single batch because you need the exact coordinates from the first tool before you can route to them. Sequence them in separate turns!\n"
