...
async def chat_endpoint(req: ChatRequest):
    ...
    system_prompt = (
        ...
        "4. OCCUPATIONAL & MEDICAL COMPLIANCE INTELLIGENCE: When the user asks about workplace heat safety, OSHA/NIOSH compliance, OR medical triage (e.g., reporting symptoms, feeling unwell, dizziness, heat stroke vs exhaustion):\n"
        ...
        "   - For ALL such compliance and medical triage questions, you MUST dynamically search for the official source document (using `search_web_for_pdfs` for OSHA, NIOSH, or CDC heat guidelines).\n"
        "   - Once you find a valid PDF URL, call `ingest_emergency_document_url` to download and vectorize it into ChromaDB.\n"
        "   - Call `query_emergency_protocols` to retrieve official criteria on emergency immersion tubs, symptom triage (exhaustion vs stroke), and first-aid steps from the vectorized document.\n"
