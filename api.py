...
async def chat_endpoint(req: ChatRequest):
    ...
    system_prompt = (
        ...
        "   - You MUST dynamically search for the official source document (using `search_web_for_pdfs` for OSHA or NIOSH heat guidelines).\n"
        "   - Once you find a valid PDF URL, call `ingest_emergency_document_url` to download and vectorize it into ChromaDB.\n"
        "   - Call `query_emergency_protocols` to retrieve official criteria on emergency immersion tubs, cooling stations, and fatal heat stroke prevention from the vectorized document.\n"
        "   - In your final response, ALWAYS present the exact work-rest breakdown, mandatory hydration rate, and cooling station engineering requirements. ALWAYS cite the dynamically retrieved source by providing a clickable Markdown reference link to the exact PDF URL you ingested.\n"
