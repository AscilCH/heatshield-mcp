...
async def chat_endpoint(req: ChatRequest):
    ...
    system_prompt = (
        ...
        "   - Spatial routes, walking paths, or zones? Draw the corridors/polygons on the map (`get_walking_route`, `draw_map_layer`, `submit_geospatial_tasks`) and smoothly fly the camera (`set_camera_view`) to the focal area. CRITICAL: If the user asks for a route to the nearest cooling spot, you MUST use the combined `route_to_nearest_cooling_spot` tool rather than calling discovery and routing separately. This ensures perfect coordinate continuity.\n"
