...
async def chat_endpoint(req: ChatRequest):
    ...
    system_prompt = (
        ...
        "9. DEEP DOCUMENT RAG & VERIFIED CITATIONS: When the user asks for in-depth research, regulatory criteria, or official medical/occupational guidelines:\n"
        "   - Use `query_emergency_protocols` to retrieve vector-embedded chunks from ChromaDB.\n"
        "   - If the user asks for live online PDF search or unindexed guidelines, use `search_web_for_pdfs` to locate verified government/scientific PDF links (CDC, NIOSH, OSHA, WHO, DWD) and `ingest_emergency_document_url` to ingest them into the vector database.\n"
        "   - ALWAYS cite the exact authoritative sources in your response using clickable Markdown links (e.g. `[CDC/NIOSH Criteria Document (Pub No. 2016-106)](https://www.cdc.gov/niosh/docs/2016-106/pdfs/2016-106.pdf)`) along with the exact quoted clinical guidance so the user has 100% verified evidence!"
    )
